...
import {relativeRangeRate, DopplerModel} from './dopplerFactor.js';
...
class Spectroplot {
    ...
    */
    measurementsAdd(dt, t, f, f0) {
        ...
        let cell_f = row.insertCell(1);
        let cell_t = row.insertCell(2);
        let cell_f0 = row.insertCell(3);

        cell_dt.innerHTML = `${dt.toFixed(1)}`;
        cell_f.innerHTML = `${f.toFixed(3)}`;
        cell_t.innerHTML = `${t.toString()}`;
        cell_f0.innerHTML = `${f0.toFixed(3)}`;
        // Update object
        this.measurements.push({f: f.toFixed(3), dt: dt.toFixed(1), f0: f0.toFixed(3)});
    }
    ...
    */
    enableCanvasClick() {
        ...
        guides.addEventListener('click', function(e) {
            const width = spectro.turnFlip ? spectro.height || spectro.fftN : spectro.width;
            const fft_height = spectro.turnFlip ? spectro.width : spectro.height || spectro.fftN;
            ...
            const f = (x / width - 0.5 ) * spectro.sample_rate / 1000; // in kHz
            const dt = y / fft_height * spectro.metadata.total_time; // in s


            const t = new Date(spectro.metadata.start_time.getTime() + 1000 * dt);
            const f0 = spectro.getUncorrectedFrequency(t);

            spectro.measurementsAdd(dt, t, f, f0);
        });
    }
    /**
        Set a color theme.
    ...
    */
    ...
    drawSpectrogramBottomAxis() {
        ...
        const time_scale = autorange(this.metadata.total_time, 10.0);
        const total_time_scaled = this.metadata.total_time / time_scale.scale;
    ...
    getUncorrectedFrequency(t) {
        const tle = document.getElementById("inputtle").value.split("\n");

        // NOTE: dateString parsing like this is strongly discouraged, see
        // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/Date
        const obsMetadata = JSON.parse(document.getElementById("inputmetadata").value);


        const observer = {
            longitude: obsMetadata.longitude,
            latitude: obsMetadata.latitude,
            height: obsMetadata.elevation,
        };

        let doppler = new DopplerModel(tle,
                                       observer,
                                       this.metadata.start_time);

        return obsMetadata.frequency * (1 - doppler.getDopplerFactor(t));
    }
    ...
    drawOverlay() {
        ...
        // NOTE: DUPLICATED from getUncorrectedFrequency!
        // NOTE: dateString parsing like this is strongly discouraged, see
        // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/Date
        const tle = document.getElementById("inputtle").value.split("\n");
        const obsMetadata = JSON.parse(document.getElementById("inputmetadata").value);
        ...
        let positionAndVelocity = satellite.propagate(satrec, this.metadata.start_time);
        let observerEcf = satellite.geodeticToEcf({
            longitude: satellite.degreesToRadians(obsMetadata.longitude),
            latitude: satellite.degreesToRadians(obsMetadata.latitude),
            height: obsMetadata.elevation / 1000,
        });
        ...
        var calcDopplerFactor = function(t) {
            // const x = (1 + Math.cos(y/height * 2 * Math.PI)) * width/2;
            ...
            return width/2 + obsMetadata.frequency * beta * width / sample_rate;
        };
        ...
        const dt = this.metadata.total_time / N; // in seconds

        ctx.moveTo(calcDopplerFactor(t), height);
        for (let i = 0; i <= N; ++i) {
            t = new Date(t.getTime() + 1000 * dt);
            ctx.lineTo(calcDopplerFactor(t), height - i/N * height);
        }
        ctx.stroke();
    }
