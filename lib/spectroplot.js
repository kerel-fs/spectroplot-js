...
class Spectroplot {
    ...
    */
    constructor(options) {
        ...
        // Enable Overlay Input Form
        document.getElementById('form-tle').style.display = null;
    }
    ...
    */
    ...
    /* Draw an overlay of a local transmitter at the given frequency offset at the beginning of the observation.
     *
     */
    drawLocalOverlay() {
        if (!this.showOverlayLocal) {
            return;
        }
        let initial_offset = document.getElementById('form-overlay-local-offset').value;

        let offset_fn = function(t) {
            return 0;
        };
        this.drawOverlay(initial_offset, offset_fn.bind(this));
    }

    /* Draw an overlay of a transmitter at the given frequency offset at the beginning of the observation
     * and the given TLE.
     *
     */
    drawSecondObjectOverlay() {
        if (!this.showOverlayRemote) {
            return;
        }
        // Initalize doppler model for the second object
        const tle_second_object = document.getElementById('form-overlay-remote-tle').value.split('\n').slice(1,3);
        // NOTE: Only basic input validation
        if (!(tle_second_object[0][0] == '1' && tle_second_object[1][0] == '2')) {
            // Input validation failed
            alert("ERROR: Input validation for TLE failed.");
            this.showOverlayRemote = false;
            return;
        }

        let second_doppler = new DopplerModel(tle_second_object,
        ...
        let initial_offset = document.getElementById('form-overlay-remote-offset').value;

        let offset_fn = function(t) {
            return this.obs_metadata.frequency * second_doppler.getDopplerFactor(t);
        };
        this.drawOverlay(initial_offset, offset_fn.bind(this));
    }

    drawOverlay(initial_offset, freq_offset_fn) {
        const canvas = document.getElementsByClassName('fft')[0];
        const ctx = canvas.getContext('2d');
        ...
        /* absolute time at waterfall start */
        const t_initial = new Date(this.wf_metadata.start_time.getTime());

        /* doppler at waterfall start */
        const df_initial =  this.obs_metadata.frequency * this.doppler.getDopplerFactor(t_initial)
                            - initial_offset * 1000;
        ...
        for (let i = 0; i <= N; ++i) {
            ...
            const dt = i/N * this.wf_metadata.total_time; // relative time in seconds
            const t = new Date(this.wf_metadata.start_time.getTime() + 1000 * dt); // absolute time
            const df =  this.obs_metadata.frequency * this.doppler.getDopplerFactor(t) - df_initial - freq_offset_fn(t);
            const dx = df/this.sample_rate * canvas.width;
